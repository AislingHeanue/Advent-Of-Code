...
// Error types
export class AocTokenMissingError extends Data.TaggedError(
  "AocTokenMissingError"
)<{
  message: string;
}> {}
  ...
  Effect.gen(function* () {
    ...
    if (!token) {
      return yield* Effect.fail(
        new AocTokenMissingError({
          message:
            "AOC_SESSION environment variable is not set. Set it to your adventofcode.com session cookie value."
        })
      );
    }
    return token;
...
export const read = (day: Day) =>
  Effect.gen(function* () {
    ...
    if (fs.existsSync(puzzlePath)) {
      ...
      yield* Console.log(
        `Puzzle file not found at "${puzzlePath}". Run download first.`
      );
    }
  });
