...
export const readInput = (day: Day): Effect.Effect<Input, Error> =>
  Effect.try({
    try: () => {
      const filePath = path.join(process.cwd(), "data", "inputs", `${formatDay(day)}.txt`)
      const raw = fs.readFileSync(filePath, "utf-8")
      return new Input(raw)
    },
    catch: (error) => new Error(`Could not read input file for day ${day}: ${error}`)
  })
...
const runPart = <T>(
  ...
  Effect.gen(function* () {
    ...
    if (result === undefined) {
      yield* Console.log(`${partName}: ${ANSI_ITALIC}not implemented${ANSI_RESET}`)
    } else {
      ...
      if (resultStr.includes("\n")) {
        ...
        yield* Console.log(`${partName}: ${ANSI_BOLD}${resultStr}${ANSI_RESET} (${formatDuration(duration)})`)
      }
    }
...
export const testDay = (day: Day, solution: DaySolution): Effect.Effect<TestResult[], unknown, any> =>
  Effect.gen(function* () {
    ...
    if (!solution.examples) {
      yield* Console.log(`Day ${formatDay(day)}: ${ANSI_ITALIC}no examples defined${ANSI_RESET}`)
      return results
    }
    ...
    if (solution.examples.part1) {
      ...
      if (passed) {
        yield* Console.log(`Day ${formatDay(day)} Part 1: ${ANSI_GREEN}PASS${ANSI_RESET}`)
      } else {
        yield* Console.log(`Day ${formatDay(day)} Part 1: ${ANSI_RED}FAIL${ANSI_RESET} (expected ${solution.examples.part1.expected}, got ${actual})`)
      }
    }
    ...
    if (solution.examples.part2) {
      ...
      if (passed) {
        yield* Console.log(`Day ${formatDay(day)} Part 2: ${ANSI_GREEN}PASS${ANSI_RESET}`)
      } else {
        yield* Console.log(`Day ${formatDay(day)} Part 2: ${ANSI_RED}FAIL${ANSI_RESET} (expected ${solution.examples.part2.expected}, got ${actual})`)
      }
    }
