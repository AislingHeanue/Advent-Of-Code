...
export class AllCommand extends Data.TaggedClass("AllCommand")<
  Record<string, never>
> {}
...
export const parseArgs = (
  args: string[]
): Effect.Effect<Command, ParseError> => {
  ...
  return Effect.gen(function* () {
    switch (command) {
      case "download": {
        ...
        if (dayStr === undefined) {
          return yield* Effect.fail(
            new ParseError({ message: "download requires a day number" })
          );
        }
      ...
      case "read": {
        ...
        if (dayStr === undefined) {
          return yield* Effect.fail(
            new ParseError({ message: "read requires a day number" })
          );
        }
        const day = yield* Effect.mapError(
          parseDay(parseInt(dayStr, 10)),
      ...
      case "solve": {
        ...
        if (dayStr === undefined) {
          return yield* Effect.fail(
            new ParseError({ message: "solve requires a day number" })
          );
        }
        const day = yield* Effect.mapError(
          parseDay(parseInt(dayStr, 10)),
