...
func partA(challenge *core.Input) int {
	return solve(challenge, 0)
}

func solve(challenge *core.Input, smudges int) int {
	...
	for i < len(lines) {
		...
		total += lookForMirror(image, smudges)
		i++
	}
...
func lookForMirror(matrix [][]rune, smudges int) int {
	height := len(matrix)
	width := len(matrix[0])
	x, found := lookForVertical(matrix, height, width, smudges)
	if found {
		// need number of the row (1-indexed) to the left of the vertical line
		return x
	}
	y, found := lookForHorizontal(matrix, height, width, smudges)
	if found {
...
// 654456..
// 987789..
func lookForVertical(matrix [][]rune, height int, width int, wantedSmudges int) (int, bool) {
	for xLeft := 0; xLeft < width-1; xLeft++ {
		...
		if mirrorValid && smudges == wantedSmudges {
			return xLeft + 1, true
		}
	}
...
func lookForHorizontal(matrix [][]rune, height int, width int, wantedSmudges int) (int, bool) {
