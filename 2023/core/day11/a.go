...
func partA(challenge *core.Input) int {
	return solve(challenge, 2)
}

func solve(challenge *core.Input, d int) int {
	...
	for i, g1 := range galaxies {
		for j, g2 := range galaxies {
			...
			total += taxicabMetric(g1, g2, rowNotEmpty, columnNotEmpty, d)
...
func taxicabMetric(p1, p2 util.Point2D, rowNotEmpty, columnNotEmpty map[int]int, d int) int {
